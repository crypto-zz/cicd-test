...
import "./RoninERC721StakingPoolMock.sol";
import "../Lib/SafeErc20/SafeErc20.sol";

contract RoninERC721StakingManagerMock is StakingManager {
    ...
    function getPendingRewards(address _user) external view returns(uint256) {
        return RoninERC721StakingPoolMock(msg.sender).stakingCount(_user) * reward;
    }

    function claimRewards(address _user) external {
        uint256 tokenAmount = RoninERC721StakingPoolMock(msg.sender).stakingCount(_user);
        address rewardToken = RoninERC721StakingPoolMock(msg.sender).getRewardToken();
